...
global BiosGetMemorySize64MBLo
BiosGetMemorySize64MBLo:
	push	ecx
	push	edx
	...
	jcxz	.use_ax_Lo			;bios may have stored it in ax,bx or cx,dx. test if cx is 0
...
global BiosGetMemorySize64MBHi
BiosGetMemorySize64MBHi:
	push	ecx
	push	edx
	...
	jcxz	.use_ax_Hi		;bios may have stored it in ax,bx or cx,dx. test if cx is 0
	mov	ax, cx			;its not, so it should contain mem size; store it
	mov	bx, dx
    mov ax, bx
    pop	edx			;mem size is in ax and bx already, return it
